...
def insert_to_db2(df):
    try:
        ...
        print(df)
        # Insert data into PostgreSQL table
        df.to_sql(TABLE_NAME2, engine, if_exists="replace", index=False, dtype={"modified_date": sql_date})
        print(f"✅ Data successfully inserted into {TABLE_NAME2}.")
    except Exception as e:
        print(f"❌ Error inserting data: {e}")
        raise
